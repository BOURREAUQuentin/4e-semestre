...
public class Clavier extends TilePane{
    ...
    public Clavier(String touches, EventHandler<ActionEvent> actionTouches, int tailleLigne) {
        ...
        // a changer pour retourner à la ligne suivant tailleLigne
        ...
        for (char lettre : touches.toCharArray()) {
            ...
            bouton.setShape(new Circle(10));
            bouton.setPrefWidth(55);
    ...
    /**
     * permet de désactiver le clavier (touches du clavier)
     */
    public void desactiveClavier(){
        for (Button bouton : this.clavier) {
            bouton.setDisable(true);
        }
    }
    ...
    /**
     * Change la couleur des boutons du clavier
     * @param couleurClavier la nouvelle couleur du clavier
     */
    public void changeCouleurClavier(String couleurClavier){
        for (Button bouton : this.clavier) {
            bouton.setStyle("-fx-background-color: "+couleurClavier);
        }
    }
}